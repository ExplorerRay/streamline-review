...
rs = openpyxl.load_workbook('result2.xlsx')
shtrs = rs.worksheets[0]

doc = docx.Document('1-32.docx')
tb = doc.tables[0]
...
for c in range(crs):
    ...
    tb.rows[2].cells[6].text = str(shtrs.cell(row=5+c*3, column=11).value) #設備
    tb.rows[2].cells[7].text = str(shtrs.cell(row=5+c*3, column=12).value) #小計
    ...
    tb.rows[3].cells[6].text = str(shtrs.cell(row=6+c*3, column=11).value) #設備
    tb.rows[3].cells[7].text = str(shtrs.cell(row=6+c*3, column=12).value) #小計
    ...
    # 業界或校外講師參與教學情形
    tb.rows[8].cells[2].text = shtrs.cell(row=5+c*3, column=22).value

    doc.save(str(shtrs.cell(row=5+c*3, column=7).value)+'備課情形期中查核表.docx')
