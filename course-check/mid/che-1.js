function createNewForm2(){
  let yr = '112'; //年度記得修改
  // 上學期
  let new_form_2 = FormApp.create(yr + "-1 課程查核"); 
...
function SetForm2(form, yr){
  ...
  form.addTextItem().setTitle('預計修課人次')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('預計專題作品數 (若無寫0)').setRequired(true).setValidation(numValidation);
  form.addParagraphTextItem().setTitle('質化成效說明').setRequired(true)
  .setHelpText("EX:1.藉由跨領域合作，淬鍊學生能力，協助產業升級，並提升學生職場競爭力。 2.強化產業連結與產學合作，創造實習與就業機會。...");

  form.addSectionHeaderItem().setTitle('目前已達成值');
  form.addTextItem().setTitle('大學部男性修課人次 (若無寫0)')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('大學部女性修課人次 (若無寫0)')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('碩士班男性修課人次 (若無寫0)')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('碩士班女性修課人次 (若無寫0)')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('博士班男性修課人次 (若無寫0)')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('博士班女性修課人次 (若無寫0)')
                    .setValidation(numValidation)
                    .setRequired(true);
  form.addTextItem().setTitle('專題作品數 (若無寫0)').setRequired(true).setValidation(numValidation);
  form.addParagraphTextItem().setTitle('質化成效說明').setRequired(true);

  // 第五點
  form.addPageBreakItem().setTitle('5. 參與聯盟活動、競賽情形');
  form.addSectionHeaderItem().setTitle('申請書內目標值');
  form.addTextItem().setTitle('預計參與聯盟相關課程推廣研習、座談之人次').setRequired(true).setValidation(numValidation);
  form.addTextItem().setTitle('預計參與聯盟相關課程推廣研習、座談之場次').setRequired(true).setValidation(numValidation);
  form.addTextItem().setTitle('預計參與聯盟相關競賽學生人數').setRequired(true).setValidation(numValidation);
  form.addParagraphTextItem().setTitle('其他');

  form.addSectionHeaderItem().setTitle('目前已達成值');
  form.addTextItem().setTitle('參與聯盟相關課程推廣研習、座談之人次').setRequired(true).setValidation(numValidation);
  form.addTextItem().setTitle('參與聯盟相關課程推廣研習、座談之場次').setRequired(true).setValidation(numValidation);
  form.addTextItem().setTitle('參與聯盟相關競賽學生人數').setRequired(true).setValidation(numValidation);
  form.addParagraphTextItem().setTitle('其他');
